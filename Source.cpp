...
int main() {
    ...
    for (int index = 0; index < a; index++) {
        ...
        findMostSimilars(results, MainDoc, SimilarDoc); //Finding most similar 5 sentence
        documentSimilarity(MainDoc, SimilarDoc); //Calculating similarity rate of the documents
        SimilarDoc.~vector();
        results.~vector();
    }
    stop = clock();
